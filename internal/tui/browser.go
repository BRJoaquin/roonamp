...
func (b *browserModel) selectCurrent() tea.Cmd {
	...
	if item.Hint == hintArtistAlbums {
		...
		session := b.session
		return func() tea.Msg {
			page, err := openAlbumsForArtist(client, zoneID, artistName, session)
			if err != nil {
	...
	return func() tea.Msg {
		...
		if len(lr.Items) == 0 {
			if artistName != "" {
				...
				if page, terr := openAlbumsForArtist(client, zoneID, artistName, "synthetic"); terr == nil {
...
func openAlbumsForArtist(client *roon.Client, zoneID, artistName, session string) (*roon.LoadResponse, error) {
	...
	albums, err := client.Load(roon.LoadRequest{
		Hierarchy:       "browse",
		Offset:          0,
		Count:           100,
		MultiSessionKey: session,
...
func pickItemByTitle(items []roon.BrowseItem, title string) string {
	for i := range items {
		it := &items[i]
		if it.ItemKey != nil && strings.EqualFold(it.Title, title) {
			return *it.ItemKey
		}
	}
	return ""
...
func pickItemByTitlePrefix(items []roon.BrowseItem, prefix string) string {
	for i := range items {
		it := &items[i]
		if it.ItemKey != nil && strings.HasPrefix(it.Title, prefix) {
			return *it.ItemKey
		}
	}
	return ""
...
func (b *browserModel) searchCmd(query string) tea.Cmd {
	...
	return func() tea.Msg {
		...
		searchKey, err := findSearchKey(client, zoneID, "")
		if err != nil {
		...
		if err != nil {
			return browseResultMsg{fromSearch: true, err: fmt.Errorf("load results: %w", err)}
		}
...
func findSearchKey(client *roon.Client, zoneID, session string) (string, error) {
	root, err := client.Load(roon.LoadRequest{
		Hierarchy:       "browse",
		Offset:          0,
		Count:           100,
		MultiSessionKey: session,
	...
	if _, err := client.Browse(roon.BrowseRequest{
		...
		return "", fmt.Errorf("browse Library: %w", err)
	}
	lib, err := client.Load(roon.LoadRequest{
		Hierarchy:       "browse",
		Offset:          0,
		Count:           100,
		MultiSessionKey: session,
...
func pickSearchKey(items []roon.BrowseItem) string {
	for i := range items {
		...
		if it.InputPrompt != nil || it.Title == "Search" {
			return *it.ItemKey
		}
	}
	return ""
